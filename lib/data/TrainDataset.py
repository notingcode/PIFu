...
class TrainDataset(Dataset):
    ...
    def __init__(self, opt, phase='train'):
        ...
        self.yaw_list = list(
            range(0, 360, 1)) if self.opt.yaw_list is None else self.opt.yaw_list
        self.pitch_list = [0]
    ...
    def get_render(self, subject, num_views, yid=0, random_sample=False):
        ...
        if random_sample:
            view_ids = np.random.choice(
                self.yaw_list, num_views, replace=False)
        ...
        for vid in view_ids:
            ...
            render_path = os.path.join(self.RENDER, subject, f'{vid:02d}.jpg')
            mask_path = os.path.join(self.MASK, subject, f'{vid:02d}.png')
    ...
    def select_sampling_method(self, subject):
        ...
        labels = np.concatenate(
            [np.ones((1, inside_points.shape[0])), np.zeros((1, outside_points.shape[0]))], 1)
    ...
    # This function is only called with when `--num_sample_color > 0 (default: 0)`
    def get_color_sampling(self, subject, yid, pid=0):
        ...
        uv_normal_path = os.path.join(
            self.UV_NORMAL, subject, '%02d.png' % (0))
        ...
        samples = torch.Tensor(surface_points).float() \
            + torch.normal(mean=torch.zeros((1, normal.size(1))),
                           std=self.opt.sigma).expand_as(normal) * normal
    ...
    def get_item(self, index):
        ...
        # get name(id) of the subject in '#####' format
        subject = self.subjects[sid]
        res = {
            'name': subject,
            'mesh_path': os.path.join(self.OBJ, subject, f'{subject}.obj'),
            'sid': sid,
            'b_min': self.B_MIN,
            'b_max': self.B_MAX,
        }
